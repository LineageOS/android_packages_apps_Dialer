...
public class CallStatsDetails implements Parcelable {
  ...
  public CallStatsDetails(CharSequence number, int numberPresentation,
    ...
    if (info != null) {
      updateFromInfo(info, ContactsPreferences.DISPLAY_ORDER_PRIMARY);
    }
  }

  public void updateFromInfo(ContactInfo info, int nameDisplayOrder) {
    if (nameDisplayOrder == ContactsPreferences.DISPLAY_ORDER_PRIMARY
        || TextUtils.isEmpty(info.nameAlternative)) {
      this.name = info.name;
    } else {
      this.name = info.nameAlternative;
    }
