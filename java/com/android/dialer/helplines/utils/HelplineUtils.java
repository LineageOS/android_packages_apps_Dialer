...
public class HelplineUtils {
    ...
    public static final String CATEGORY_STR_FORMAT = "helpline_category_%s";
    public static final String LANGUAGE_STR_FORMAT = "helpline_language_%s";

    /* Get the name of the helpline, fall back to the number if not given */
    public static String getName(Resources res, SensitivePhoneNumberInfo info) {
        if (info != null) {
            String name = info.get("name");
            return !TextUtils.isEmpty(name) ? name : info.get("number");
        }
    ...
    public static String getCategories(Resources res, SensitivePhoneNumberInfo info) {
        if (info != null) {
