...
public class DeepLinkCache {
     ...
     */
    private final HashMap<Uri, PendingResult<DeepLink.DeepLinkResultList>> pendingRequests;
    /**
    ...
    public DeepLinkCache(DeepLinkListener listener) {
        ...
        pendingRequests = new HashMap<Uri, PendingResult<DeepLink.DeepLinkResultList>>();
    ...
    private void queryDeepLinks(DeepLinkRequest request) {
        synchronized (pendingRequests) {
            pendingRequests.put(request.getUris().get(0), DeepLinkIntegrationManager.getInstance()
                    .getPreferredLinksForList(mDeepLinkCallback, DeepLinkContentType.CALL,
                            request.getUris()));
        }
    }
    ...
    public void clearPendingQueries(String number, long[] calltimes) {
        synchronized (pendingRequests) {
            ...
            if (pendingRequests.containsKey(uri)) {
                PendingResult<DeepLink.DeepLinkResultList> request = pendingRequests.remove(uri);
                request.cancel();
            }
        }
    }
