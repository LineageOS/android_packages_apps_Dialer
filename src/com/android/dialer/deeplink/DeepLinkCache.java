...
public class DeepLinkCache {
    ...
    private static final int CLEAR_UNEEDED_QUERIES = 2;
    ...
    private final LinkedList<DeepLinkRequest> toIgnore;
                ...
                synchronized (toIgnore) {
                    if(!toIgnore.isEmpty()) {
                        synchronized (mRequests) {
                            for(DeepLinkRequest request: toIgnore) {
    ...
    public DeepLinkCache(DeepLinkListener listener) {
        ...
        toIgnore = new LinkedList<DeepLinkRequest>();
    ...
    public void clearPendingQueries(String number, long[] calltimes) {
        List<Uri> ignoring = new ArrayList<Uri>();
        for(long time: calltimes) {
            ignoring.add(DeepLinkIntegrationManager.generateCallUri
                    (number, time));
        }
        DeepLinkRequest ignoreThis = new DeepLinkRequest(ignoring);
        synchronized (toIgnore) {
            toIgnore.add(ignoreThis);
        }
    }
}
