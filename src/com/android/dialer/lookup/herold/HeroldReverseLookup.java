/*
 * Copyright (C) 2015 Raphael Müllner The CyanogenMod Project
 *
...
public class HeroldReverseLookup extends ReverseLookup {
    private static final String TAG = HeroldReverseLookup.class.getSimpleName();

    public HeroldReverseLookup(Context context) {
    }
    ...
    public ContactInfo lookupNumber(Context context,
        ...
        if (normalizedNumber.startsWith("+") && !normalizedNumber.startsWith("+43")) {
            // Herold only supports Austrian numbers
            return null;
        }

        HeroldApi.ContactInfo info = HeroldApi.reverseLookup(context, normalizedNumber);
        if (info == null) {
            return null;
        }
